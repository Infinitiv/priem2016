class BenefitDocumentsController < ApplicationController
  ...
  def update
    ...
    value_name = 'status_update'
    old_value = @benefit_document.entrant_application.status
    new_value = 'внесены изменения'
    Journal.create(user_id: current_user.id, entrant_application_id: @benefit_document.entrant_application.id, method: __method__.to_s, value_name: value_name, old_value: old_value, new_value: new_value)
