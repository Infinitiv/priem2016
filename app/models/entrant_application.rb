...
#     errors[:not_agreed_target_entrants] = find_not_agreed_entrants(applications)
  ...
  def generate_contract(competitive_group_id)
...
#     получение справочника дисциплин
    ...
    pdf.text '7.4. Настоящий Договор составлен в 3 экземплярах, по одному для каждой из сторон. Все экземпляры имеют одинаковую юридическую силу. Изменения и дополнения настоящего Договора могут производиться только в письменной форме и подписываться уполномоченными представителями Сторон.', size: 11, align: :justify
    ...
    pdf.grid([1, 0], [4, 0]).bounding_box do
      ...
      pdf.text '________/Борзов Е. В./', size: 11
      pdf.move_down 10
    ...
    pdf.grid([1, 2], [4, 2]).bounding_box do
      ...
      pdf.text "#{fio}", size: 11
      pdf.text "#{birth_date}", size: 11
      pdf.text "#{verified_address ? verified_address : address}", size: 11
      pdf.text "Паспорт #{identity_documents.last.identity_document_data}", size: 11
