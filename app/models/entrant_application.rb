...
#     errors[:not_agreed_target_entrants] = find_not_agreed_entrants(applications)
  ...
  def generate_contract
    ...
    pdf.text 'направление подготовки, специальность/направленность: ___________ по очной форме обучения в пределах федерального государственного образовательного стандарта в соответствии с учебными планами и образовательными программами Исполнителя.', size: 11, align: :justify
    pdf.text '1.2. Срок освоения образовательной программы (продолжительность обучения) на момент подписания договора составляет _______ лет.', size: 11, align: :justify
    ...
    pdf.move_down 6
    pdf.text '3.1. Полная стоимость образовательных услуг за весь период обучения Обучающегося составляет ___ рублей.', size: 11, align: :justify
    pdf.text 'Стоимость за первый год обучения на момент заключения настоящего Договора составляет ____ рублей.', size: 11, align: :justify
    ...
    pdf.text '7.4. Настоящий Договор составлен в ___ экземплярах, по одному для каждой из сторон. Все экземпляры имеют одинаковую юридическую силу. Изменения и дополнения настоящего Договора могут производиться только в письменной форме и подписываться уполномоченными представителями Сторон.', size: 11, align: :justify
    ...
    attachment = attachments.where(document_type: 'contract', document_id: id, template: true).first || Attachment.new
    attachment.entrant_application_id = id
