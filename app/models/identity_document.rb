...
class IdentityDocument < ActiveRecord::Base
  ...
  def identity_document_data
    "Серия #{identity_document_series} номер #{identity_document_number}, выдан #{identity_document_date.strftime("%d.%m.%Y") if identity_document_date} #{identity_document_issuer}"
  end
