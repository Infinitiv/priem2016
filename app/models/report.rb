class Report
  def self.mon(admission_volumes)
    points = {}
    admission_volumes.each do |admission_volume|
      direction_id = admission_volume.direction_id
      points[direction_id] = {}
      points[direction_id][:f2_2] = {}
      points[direction_id][:f2_3] = {}
      competitive_groups = CompetitiveGroup.where(direction_id: direction_id, education_source_id: 20)
      entrant_applications = EntrantApplication.where(enrolled: competitive_groups.map(&:id))
      ...
      competitive_groups = CompetitiveGroup.where(direction_id: direction_id, education_source_id: 16)
      entrant_applications = EntrantApplication.where(enrolled: competitive_groups.map(&:id))
      marks = Mark.joins(:entrant_application).where(entrant_applications: {id: entrant_applications.map(&:id)}, form: 'ЕГЭ')
      mean = (marks.sum(:value).to_f/marks.count).round(2)
      ...
      entrant_applications = EntrantApplication.where(enrolled: competitive_groups.map(&:id), olympionic: false)
      marks = Mark.joins(:entrant_application).where(entrant_applications: {id: entrant_applications.map(&:id)}, form: 'ЕГЭ')
      mean = (marks.sum(:value).to_f/marks.count).round(2)
      ...
      entrant_applications = EntrantApplication.where(enrolled: competitive_groups.map(&:id)).joins(:institution_achievements).where(institution_achievements: {id_category: [9, 15, 16]})
      points[direction_id][:f2_3][:p2_30] = entrant_applications.count
      
      
      entrant_applications = EntrantApplication.where(enrolled: competitive_groups.map(&:id)).joins(:institution_achievements).where(institution_achievements: {id_category: 17})
